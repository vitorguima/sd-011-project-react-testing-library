  ...
  render() {
    return (
      <div className="App">
        <h1>Pokédex</h1>
        <nav>
          <Link className="link" data-testid="link" to="/">{`Home`}</Link>
          <Link className="link" data-testid="link" to="/about">{`About`}</Link>
          <Link className="link" data-testid="link" to="/favorites">{`Favorite Pokémons`}</Link>
        </nav>
        {this.renderRoutes()}
      </div>
    );
