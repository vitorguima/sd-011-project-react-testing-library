...
const typeButtonsName = ['Electric', 'Fire', 'Bug', 'Poison','Psychic', 'Normal', 'Dragon'];
...
const pokemonExtra = [{
  ...
  image: 'https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.ofuxico.com.br%2Ffamosos%2Frubens-barrichello%2F&psig=AOvVaw3Qi2664Q3Du4HA8PDIfhoK&ust=1624758424701000&source=images&cd=vfe&ved=0CAoQjRxqFwoTCMj5uPmWtPECFQAAAAAdAAAAABAD',
...
describe('Testes do componente <Pokedex.js />', () => {
  ...
  it('Teste se é mostrado apenas um Pokémon por vez', () => {
    const { getByText, getAllByTestId } = render(<Router>
      <Pokedex pokemons={ pokemons } isPokemonFavoriteById= { isPokemonFavoriteById } />
    </Router>
    );
    const btnProximo = getByText(/Próximo pokémon/i);
  ...
  it('Teste se a Pokédex tem os botões de filtro', () => {
    const { getAllByTestId, getByTestId } = render(<Router>
      <Pokedex pokemons={ pokemons } isPokemonFavoriteById= { isPokemonFavoriteById } />
    </Router>
    );
  ...
  it('Teste se é criado, dinamicamente, um botão de filtro para cada tipo de Pokémon', () => {
    ...
    const { getByText, getAllByTestId, getByTestId } = render(<Router>
      <Pokedex pokemons={ pokemons } isPokemonFavoriteById= { isPokemonFavoriteById } />
    </Router>
    );
